/**
 * WBG Neuhaus Scraper – Supabase Edge Function
 *
 * Die Wohnbaugenossenschaft Neuhaus (WBG Neuhaus) besitzt Wohnungen in den
 * Gemeinden Köniz, Schliern und Wabern südlich von Bern. Da alle Objekte
 * im Grossraum Bern liegen, wird kein PLZ-Filter angewendet.
 * Die Scraping-Reihenfolge ist:
 *   1. JSON-LD (strukturierte Daten im HTML-Head)
 *   2. HTML-Blöcke (article/div/li mit typischen CSS-Klassen inkl. "angebot")
 */
...
const SOURCE_NAME = 'wbg_neuhaus';
const BASE_URL = 'https://www.wbg-neuhaus.ch';
const SEARCH_URL = 'https://www.wbg-neuhaus.ch/freie-wohnungen/';
...
// Listing aus einem HTML-Block (article, div oder li) extrahieren
...
async function scrape(): Promise<ScrapedListing[]> {
  ...
  // ── 2. Versuch: HTML-Blöcke (article, div, li) ──────────
  if (listings.length === 0) {
    // Probiere verschiedene Container-Elemente; "angebot" ist typisch für Genossenschaftsseiten
    const patterns: Array<[RegExp, string]> = [
      [/<article[^>]*class="[^"]*(?:wohnung|listing|objekt|item|angebot)[^"]*"/i, 'article'],
      [/<div[^>]*class="[^"]*(?:wohnung|listing|objekt|item|angebot)[^"]*"/i, 'div'],
      [/<li[^>]*class="[^"]*(?:wohnung|listing|objekt|item|angebot)[^"]*"/i, 'li'],
    ];

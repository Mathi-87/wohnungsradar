/**
 * EBG Bern Scraper – Supabase Edge Function
 *
 * Die Eisenbahner-Baugenossenschaft Bern (EBG) verwaltet rund 700 Wohnungen
 * vorwiegend in der Stadt Bern und im Agglo-Gebiet. Da alle Objekte in Bern
 * liegen, wird kein PLZ-Filter angewendet. Die Scraping-Reihenfolge ist:
 *   1. JSON-LD (strukturierte Daten im HTML-Head)
 *   2. HTML-Blöcke (article/div/li mit typischen CSS-Klassen)
 */
...
const SOURCE_NAME = 'ebg_bern';
const BASE_URL = 'https://www.ebgbern.ch';
const SEARCH_URL = 'https://www.ebgbern.ch/freie-wohnungen/';
...
async function scrape(): Promise<ScrapedListing[]> {
  ...
  if (listings.length === 0) {
    // Probiere verschiedene Container-Elemente mit typischen CSS-Klassen
    const patterns: Array<[RegExp, string]> = [
      [/<article[^>]*class="[^"]*(?:wohnung|listing|objekt|item|property)[^"]*"/i, 'article'],
      [/<div[^>]*class="[^"]*(?:wohnung|listing|objekt|item|property)[^"]*"/i, 'div'],
      [/<li[^>]*class="[^"]*(?:wohnung|listing|objekt|item|property)[^"]*"/i, 'li'],
    ];
